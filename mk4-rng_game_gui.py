...
# ---------------------------------------------------------------------------
# Palette (dark cosmic void + gold sun-flare accents — aura-game inspired)
# ---------------------------------------------------------------------------
...
PALETTE = {
    ...
    "text":     "#f5efe0",
    "subtext":  "#9a8fc2",
    "accent":   "#b98bff",   # violet sun-flare
    "accent_hover": "#cba6ff",
    "green":    "#6ee7a0",
    "blue":     "#5cc4ff",
    "yellow":   "#ffcf5c",
...
# ---------------------------------------------------------------------------
# Fishing minigame — pixel-art pond. Fish pop up at random spots and must
# be clicked before they swim off; catches pay coins or a temporary luck
# blessing (reuses the same boost dict shape as shop potions).
# ---------------------------------------------------------------------------
FISH_PX = 8                              # pixel "block" size for sprite art
FISHING_CANVAS_SIZE = (640, 360)
FISH_SPAWN_MS_RANGE = (700, 1600)        # gap between spawns
FISH_LIFETIME_MS = 2600                  # a fish swims off if not caught in time

WATER_LIGHT = "#1e4d6b"
WATER_DARK = "#173d57"

FISH_TYPES = [
    {"name": "Minnow",           "weight": 45,  "color": PALETTE["subtext"],
     "reward": {"kind": "coins", "amount": 2}},
    {"name": "Perch",            "weight": 28,  "color": PALETTE["green"],
     "reward": {"kind": "coins", "amount": 5}},
    {"name": "Koi",              "weight": 15,  "color": PALETTE["peach"],
     "reward": {"kind": "coins", "amount": 12}},
    {"name": "Golden Trout",     "weight": 7,   "color": PALETTE["yellow"],
     "reward": {"kind": "boost", "scope": "Good+", "boost_pct": 15, "rolls": 8}},
    {"name": "Rainbow Marlin",   "weight": 3.5, "color": PALETTE["accent"],
     "reward": {"kind": "boost", "scope": "Rare+", "boost_pct": 25, "rolls": 6}},
    {"name": "Ancient Leviathan", "weight": 1,  "color": PALETTE["sapphire"],
     "reward": {"kind": "boost", "scope": "Legendary+", "boost_pct": 50, "rolls": 4}},
]

# Pixel-block sprites: (dx, dy, color_key) offsets in FISH_PX units from center.
BOAT_SPRITE = [
    (-1, -3, "mast"), (-1, -2, "mast"), (-1, -1, "mast"),
    (0, -2, "flag"), (1, -2, "flag"),
    (-3, 0, "hull"), (-2, 0, "hull"), (-1, 0, "hull"), (0, 0, "hull"),
    (1, 0, "hull"), (2, 0, "hull"), (3, 0, "hull"),
    (-2, 1, "hull"), (-1, 1, "hull"), (0, 1, "hull"), (1, 1, "hull"), (2, 1, "hull"),
    (-1, 2, "hull_dark"), (0, 2, "hull_dark"), (1, 2, "hull_dark"),
]
BOAT_COLORS = {"mast": "#8b6b4a", "flag": PALETTE["red"], "hull": "#a9713f", "hull_dark": "#7a5230"}

FISH_SPRITE = [
    (-1, -1, "body"), (0, -1, "body"), (1, -1, "body"),
    (-2, 0, "body"), (-1, 0, "body"), (0, 0, "body"), (1, 0, "body"),
    (-1, 1, "body"), (0, 1, "body"), (1, 1, "body"),
    (-3, -1, "tail"), (-3, 0, "tail"), (-3, 1, "tail"),
    (1, -1, "eye"),
]
...
def make_badge(parent, text, color, bg, fg=PALETTE["mantle"]):
    ...
    w, h = tmp.winfo_reqwidth() + 30, tmp.winfo_reqheight() + 14
    tmp.destroy()
...
class RNGGameApp:
    def __init__(self, root):
        ...
        self.quick_roll_var = tk.BooleanVar(value=False)  # skip spin/cutscene animations
        self.auto_roll_var = tk.BooleanVar(value=False)   # keep rolling until stopped/out of coins
        self._auto_roll_job = None
        ...
        self.fishing_sprites = {}   # fish_id -> {tag, type, expire_job}
        self._fish_spawn_job = None
    ...
    def _configure_ttk_style(self):
        ...
        style.configure(
            ...
            padding=[22, 14], font=("Segoe UI", 12, "bold"), borderwidth=0,
        )
        style.map(
            "TNotebook.Tab",
    ...
    def _build_notebook(self):
        ...
        self.fishing_tab = tk.Frame(self.notebook, bg=PALETTE["base"])
    ...
    def _build_roll_tab(self):
        ...
        # Result card, with a starfield + soft ambient glow layered behind
        # it that recolors to match whatever rarity was just rolled.
        glow_wrap = tk.Frame(frame, bg=PALETTE["base"])
        glow_wrap.pack(fill="x", padx=20, pady=(14, 14))

        self.glow_canvas = tk.Canvas(glow_wrap, height=230, bg=PALETTE["base"], highlightthickness=0, bd=0)
        self.glow_canvas.pack(fill="both", expand=True)
        self._glow_color = PALETTE["accent"]
        self.glow_canvas.bind("<Configure>", self._on_glow_canvas_configure)
        ...
        result_pad = tk.Frame(result_body, bg=PALETTE["surface0"], padx=36, pady=30)
        result_pad.pack()
        ...
        self.result_label = tk.Label(
            result_pad, textvariable=self.result_var, font=("Segoe UI", 20, "bold"),
            bg=PALETTE["surface0"], fg=PALETTE["text"], pady=14, wraplength=520,
        )
        ...
        self.roll_btn = SmoothButton(
            ...
            bg=PALETTE["accent"], hover_bg=PALETTE["accent_hover"], radius=16,
            font=("Segoe UI", 14, "bold"), padx=26, pady=15,
        )
        ...
        self.roll10_btn = SmoothButton(
            ...
            bg=PALETTE["peach"], hover_bg=blend_color(PALETTE["peach"], PALETTE["white"], 0.25), radius=16,
            font=("Segoe UI", 12, "bold"), padx=20, pady=15,
        )
        self.roll10_btn.grid(row=0, column=1, padx=8)

        # Quick Roll / Auto Roll toggles — skip animations and/or keep
        # rolling hands-free, for grinding through rolls faster.
        toggles = tk.Frame(frame, bg=PALETTE["base"])
        toggles.pack(pady=(0, 12))
        for text, var, cmd in (
            ("⚡ Quick Roll", self.quick_roll_var, None),
            ("🔁 Auto Roll", self.auto_roll_var, self._toggle_auto_roll),
        ):
            tk.Checkbutton(
                toggles, text=text, variable=var, command=cmd,
                bg=PALETTE["base"], fg=PALETTE["text"], selectcolor=PALETTE["surface0"],
    ...
    def _finish_spin_animation(self):
        ...
        # Quick Roll skips cutscenes too, so auto-rolling stays fast.
        cutscene_pulls = [] if quick else [(r, i) for r, i, _ in results if r in CUTSCENE_CONFIG]
        if cutscene_pulls:
            self._cutscene_queue = cutscene_pulls
            self._play_next_cutscene()
        else:
            self._set_buttons_enabled(True)
            self._continue_auto_roll()
    ...
    def _on_glow_canvas_configure(self, _event=None):
        self._draw_starfield()
        self._draw_ambient_glow(self._glow_color)

    def _draw_starfield(self):
        """A scattering of small static 'stars' behind the result card,
        drawn once, to set a cosmic backdrop for the roll screen."""
        canvas = self.glow_canvas
        if canvas.find_withtag("stars"):
            return
        ...
        for _ in range(60):
            ...
            shade = blend_color(PALETTE["text"], PALETTE["base"], rng.uniform(0.35, 0.75))
            canvas.create_oval(x - r, y - r, x + r, y + r, fill=shade, outline="", tags="stars")
        canvas.tag_lower("stars")
    ...
    def _draw_ambient_glow(self, color):
        ...
        for i in range(8, 0, -1):
            rx, ry = i * 48, i * 24
            t = i / 8
            ring_color = blend_color(color, PALETTE["base"], 0.2 + t * 0.6)
            canvas.create_oval(cx - rx, cy - ry, cx + rx, cy + ry, fill=ring_color, outline="", tags="glow")
        canvas.tag_raise("glow", "stars")

    def _pop_effect(self):
        """Brief scale-up 'pop' on the final result so it feels like it lands."""
        self.result_label.config(font=("Segoe UI", 24, "bold"))
        self.root.after(140, lambda: self.result_label.config(font=("Segoe UI", 20, "bold")))
    ...
    def _toggle_auto_roll(self):
        if self.auto_roll_var.get():
            # Only kick off right away if nothing is mid-roll — if a spin
            # or cutscene is playing, the completion callback picks it up.
            if self.roll_btn._state != "disabled":
                self._continue_auto_roll()
        elif self._auto_roll_job:
            self.root.after_cancel(self._auto_roll_job)
            self._auto_roll_job = None

    def _continue_auto_roll(self):
        if not self.auto_roll_var.get():
            return
        if self.currency < ROLL_COST:
            self.auto_roll_var.set(False)
            messagebox.showinfo("Auto Roll stopped", "Out of coins!")
            return
        delay = 60 if self.quick_roll_var.get() else 350
        self._auto_roll_job = self.root.after(delay, self.do_single_roll)
    ...
    def _build_sell_top_bar(self, parent):
        ...
        btn = SmoothButton(
            ...
            bg=PALETTE["red"], hover_bg=blend_color(PALETTE["red"], PALETTE["white"], 0.25),
            fg=PALETTE["mantle"], radius=10,
        )
        btn.pack(side="right")
    ...
    def _build_fishing_tab(self):
        frame = self.fishing_tab
        tk.Label(
            frame, text="Fishing Hole", font=FONT_HEADER,
            bg=PALETTE["base"], fg=PALETTE["text"],
        ).pack(pady=(18, 4))
        tk.Label(
            frame, text="Click the fish before they swim off. Rarer catches pay bigger coin "
                        "hauls or a temporary roll-luck blessing.",
            font=FONT_SMALL, bg=PALETTE["base"], fg=PALETTE["subtext"],
            wraplength=560, justify="left",
        ).pack(pady=(0, 10))

        w, h = FISHING_CANVAS_SIZE
        canvas_wrap = tk.Frame(frame, bg=PALETTE["mantle"], padx=4, pady=4)
        canvas_wrap.pack()
        self.fish_canvas = tk.Canvas(
            canvas_wrap, width=w, height=h, bg=WATER_LIGHT, highlightthickness=0, bd=0,
        )
        self.fish_canvas.pack()
        self._draw_fishing_scenery()

        btn_row = tk.Frame(frame, bg=PALETTE["base"])
        btn_row.pack(pady=10)
        self.fish_start_btn = SmoothButton(
            btn_row, "🎣 Start Fishing", command=self._start_fishing,
            bg=PALETTE["blue"], hover_bg=blend_color(PALETTE["blue"], PALETTE["white"], 0.2), radius=12,
        )
        self.fish_start_btn.grid(row=0, column=0, padx=6)
        self.fish_stop_btn = SmoothButton(
            btn_row, "⏹ Stop Fishing", command=self._stop_fishing,
            bg=PALETTE["surface2"], hover_bg=PALETTE["surface1"], radius=12,
        )
        self.fish_stop_btn.grid(row=0, column=1, padx=6)
        self.fish_stop_btn.config(state="disabled")

        tk.Label(
            frame, text="CATCH LOG", font=("Segoe UI", 9, "bold"),
            bg=PALETTE["base"], fg=PALETTE["subtext"],
        ).pack(anchor="w", padx=22)

        log_wrap = tk.Frame(frame, bg=PALETTE["surface0"])
        log_wrap.pack(fill="both", expand=True, padx=20, pady=(4, 18))
        self.fish_log_widget = tk.Text(
            log_wrap, height=5, bg=PALETTE["surface0"], fg=PALETTE["text"],
            font=FONT_MONO, bd=0, highlightthickness=0, padx=14, pady=8,
            state="disabled", wrap="word",
        )
        self.fish_log_widget.pack(fill="both", expand=True)

    def _draw_fishing_scenery(self):
        """Pixel-block water with a little boat anchored in the middle."""
        c = self.fish_canvas
        w, h = FISHING_CANVAS_SIZE
        c.delete("scenery")
        for row in range(0, h, FISH_PX):
            shade = WATER_DARK if (row // FISH_PX) % 2 == 0 else WATER_LIGHT
            c.create_rectangle(0, row, w, row + FISH_PX, fill=shade, outline="", tags="scenery")
        self._draw_pixel_sprite(c, w // 2, h // 2, BOAT_SPRITE, BOAT_COLORS, tag="scenery")
        c.tag_lower("scenery")

    def _draw_pixel_sprite(self, canvas, cx, cy, sprite, color_map, tag):
        for dx, dy, key in sprite:
            x0, y0 = cx + dx * FISH_PX, cy + dy * FISH_PX
            canvas.create_rectangle(
                x0, y0, x0 + FISH_PX, y0 + FISH_PX, fill=color_map[key], outline="", tags=tag,
            )
    ...
    def _start_fishing(self):
        ...
        self.fish_start_btn.config(state="disabled")
        self.fish_stop_btn.config(state="normal")
        self._fish_log("— cast your line —")
        self._schedule_fish_spawn()

    def _stop_fishing(self):
        self.fishing_active = False
        if self._fish_spawn_job:
            self.root.after_cancel(self._fish_spawn_job)
            self._fish_spawn_job = None
        for fid in list(self.fishing_sprites):
            self._despawn_fish(fid)
        self.fish_start_btn.config(state="normal")
        self.fish_stop_btn.config(state="disabled")

    def _schedule_fish_spawn(self):
        if not self.fishing_active:
            return
        delay = random.randint(*FISH_SPAWN_MS_RANGE)
        self._fish_spawn_job = self.root.after(delay, self._spawn_fish)
    ...
    def _spawn_fish(self):
        ...
        fish_type = random.choices(FISH_TYPES, weights=[f["weight"] for f in FISH_TYPES], k=1)[0]
        self._fish_id_counter += 1
        fid = self._fish_id_counter
        tag = f"fish{fid}"

        color_map = {
            "body": fish_type["color"],
            "tail": blend_color(fish_type["color"], "#000000", 0.35),
            "eye": PALETTE["mantle"],
        }
        self._draw_pixel_sprite(self.fish_canvas, x, y, FISH_SPRITE, color_map, tag=tag)
        self.fish_canvas.tag_bind(tag, "<Button-1>", lambda e, fid=fid: self._catch_fish(fid))

        expire_job = self.root.after(FISH_LIFETIME_MS, lambda: self._despawn_fish(fid))
        self.fishing_sprites[fid] = {"tag": tag, "type": fish_type, "expire_job": expire_job}
        self._schedule_fish_spawn()

    def _catch_fish(self, fid):
        entry = self.fishing_sprites.get(fid)
        if not entry:
            return
        fish_type = entry["type"]
        self._despawn_fish(fid)

        reward = fish_type["reward"]
        if reward["kind"] == "coins":
            self.currency += reward["amount"]
            self._update_status_bar()
            self._fish_log(f"🎣 {fish_type['name']} — +{reward['amount']} coins")
        else:
            self.active_boosts.append({
                "name": f"{fish_type['name']} Blessing",
                "scope": reward["scope"],
                "affected": SCOPE_RARITIES[reward["scope"]],
                "multiplier": 1 + reward["boost_pct"] / 100,
                "boost_pct": reward["boost_pct"],
                "rolls_remaining": reward["rolls"],
            })
            self._refresh_boosts_strip()
            self._fish_log(
                f"🎣 {fish_type['name']} — +{reward['boost_pct']}% {reward['scope']} "
                f"odds for {reward['rolls']} rolls"
            )

    def _despawn_fish(self, fid):
        entry = self.fishing_sprites.pop(fid, None)
        if not entry:
            return
        try:
            self.root.after_cancel(entry["expire_job"])
        except Exception:
            pass
        self.fish_canvas.delete(entry["tag"])

    def _fish_log(self, text):
        if not hasattr(self, "fish_log_widget"):
            return
        self.fish_log_widget.config(state="normal")
        self.fish_log_widget.insert("end", text + "\n")
        self.fish_log_widget.see("end")
        self.fish_log_widget.config(state="disabled")
